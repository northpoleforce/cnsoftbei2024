...
public:
    ...
    static std::string          draw_objects(const cv::Mat&                                image,
                                      cv::Mat&                                      res,
...
void YOLOv8::postprocess(std::vector<Object>& objs)
{
    ...
    for (int i = 0; i < num_dets[0]; i++) {
        //TODO 以下两行用来去除置信度低于阈值的检测结果
       float conf_thres = 0.8;
       if (*(scores + i) < conf_thres) continue;
...
string YOLOv8::draw_objects(const cv::Mat&                                image,
                          cv::Mat&                                      res,
                          const std::vector<Object>&                    objs,
                            const std::vector<std::string>&               CLASS_NAMES,
                            const std::vector<std::vector<unsigned int>>& COLORS)
{
    ...
    for (auto& obj : objs) {
        ...
        if (obj.prob > now_prob) {
            now_prob = obj.prob;
            obj_cls = CLASS_NAMES[obj.label];
        }
        int      baseLine   = 0;
